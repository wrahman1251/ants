...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            for a_bee in list(self.place.bees):
                Insect.reduce_armor(self.place.bees[self.place.bees.index(a_bee)], self.damage)
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        "*** YOUR CODE HERE ***"
        # don't forget that you should never edit any list you are iterating over duh
        for bee in list(self.place.bees):
            Insect.reduce_armor(self.place.bees[self.place.bees.index(bee)], self.damage)
